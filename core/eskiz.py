...
class SendSmsApiWithEskiz:
    ...
    def send_message(self, message):
        ...
        PAYLOAD = {
            ...
            'message': message,
            'from': '4546',
            # 'callback_url': 'http://afbaf9e5a0a6.ngrok.io/sms-api-result/'   # result olish uchun
        }
...
def sent_sms(message, phone):
    try:
        sms_api = SendSmsApiWithEskiz(message=message, phone=phone)
        response = sms_api.send()
        print(response)
    except Exception as e:
        return Response({'response': f'Failed to send SMS: {str(e)}'}, 400)
